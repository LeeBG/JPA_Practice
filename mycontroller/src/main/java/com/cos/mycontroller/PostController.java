...
public class PostController {
	...
	//get방식은 데이터를 전송하는데 역할을 두는 것이 아니라 데이터를 요구하는데 목적이 있음
	//데이터를 전송(insert)을 목적으로 한다면 Post를 사용한다.
	...
	//@RequestBody어노테이션을 붙이는 순간 json데이터를 받을 수 있음
	@PostMapping("/test/post/json")
	public User testPostJson(@RequestBody User user) {	//json을 전송(x-www-form-urlencoded에서 json데이터로 받을수 있다.)
